...
- Criação de DOCS/10_PURPOSE_FIRST_INVARIANT.md.
- Definição dos 8 campos obrigatórios para ações materiais (PURPOSE, SCOPE, BENEFICIARY, EXPECTED_RESULT, ACCEPTANCE_CRITERIA, EVIDENCE, COST/RISK, PRESERVE).
- Políticas fail-closed: ESTHETIC_ONLY => HOLD, DUPLICATE_WITHOUT_NEW_CAPABILITY => HOLD, TOOL_WITHOUT_CLEAR_CONSUMER => HOLD, CHANGE_WITHOUT_ACCEPTANCE_CRITERIA => HOLD.
- Regra de não-substituição de capacidades existentes sem razão funcional + evidência de migração + zero perda.
- Atualização do ROAD_TRACE contínuo.
